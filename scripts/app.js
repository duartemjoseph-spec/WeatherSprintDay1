...
const searchCity = async (cityInput) => {
    if (!cityInput) return; // Stop if empty

    try {
        //  Find the location (Geo API) to get the State (CA)
...
const renderHourlyForecast = (data) => {
    ...
    let nextFiveHours = data.list.slice(0, 5);
    
    nextFiveHours.forEach((hour) => {
...
document.getElementById('homeBtn').addEventListener('click', () => {
    if (navigator.geolocation) {
         navigator.geolocation.getCurrentPosition(
            ...
            (error) => { searchCity('Stockton'); }
        );
    } else { searchCity('Stockton'); }
});
